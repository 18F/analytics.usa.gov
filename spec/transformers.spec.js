...
describe('transformers', () => {
  ...
  describe('extractArrayValue', () => {
    ...
    it('will error if just an object', () => {
      ...
      assert.throws(() => { transformers.extractArrayValue(badObject) }, TypeError);
    });
  });
  ...
  describe('toTopPercents', () => {
    it('returns the top percents given some data', () => {
      const resultsArray = [{ key: 'bob', value: 1000, proportion: 76.92189350933063 },
      { key: 'harry', value: 300, proportion: 23.07656805279919 },
      { key: 'Other', proportion: 0.0015384378701866124, children: [] },
      ];

      assert.deepEqual(
        transformers.toTopPercents({ totals: { os: { bob: 1000, tom: .02, harry: 300 } } }, 'os'),
        resultsArray,
      );
    });
  });
